...
pcl::visualization::PCLVisualizer::Ptr initScene(Box window, int zoom) {
    ...
    viewer->addCube(window.x_min, window.x_max, window.y_min, window.y_max, 0,
                    0, 1, 1, 1, "window");
    return viewer;
}
    ...
    if (node != NULL) {
        ...
        if (depth % 2 == 0) {
            viewer->addLine(pcl::PointXYZ(node->point[0], window.y_min, 0),
                            pcl::PointXYZ(node->point[0], window.y_max, 0), 0,
        ...
        // split on y axis
            ...
            viewer->addLine(pcl::PointXYZ(window.x_min, node->point[1], 0),
                            pcl::PointXYZ(window.x_max, node->point[1], 0), 1,
