...
void renderHighway(pcl::visualization::PCLVisualizer::Ptr& viewer) {
    ...
    viewer->addCube(-roadLength / 2, roadLength / 2, -roadWidth / 2,
                    roadWidth / 2, -roadHeight, 0, .2, .2, .2,
                    "highwayPavement");
...
void renderRays(pcl::visualization::PCLVisualizer::Ptr& viewer,
    ...
    for (pcl::PointXYZ point : cloud->points) {
        viewer->addLine(pcl::PointXYZ(origin.x, origin.y, origin.z), point, 1,
                        0, 0, "ray" + std::to_string(countRays));
        countRays++;
    }
}
